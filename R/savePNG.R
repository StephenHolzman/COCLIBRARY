savePNG <- function(plot = NULL, path = getwd(), width = 600, height = 600, title = "", subtitle = "", cite = "", author = "", headerbackground = default_headerbackground, headerfontcol = default_headerfontcol, footerbackground = default_footerbackground, footerfontcol = default_footerfontcol, titlefont = default_titlefont,logo = default_logo){
  ...
  grid.text(title, x=unit(0.01,"npc"),y=unit((height-26)/height,"npc"),just="left",gp=gpar(fontsize=36,fontfamily=titlefont,fontface="bold", col=headerfontcol))
  grid.text(subtitle, x=unit(0.01,"npc"),y=unit((height-56)/height,"npc"),just="left",gp=gpar(fontsize=18,fontfamily=titlefont,fontface="bold", col=headerfontcol))
  grid.text(cite, x=unit(0.01,"npc"),y=unit(20/height,"npc"),just="left",gp=gpar(fontsize=18,fontfamily=titlefont,fontface="bold", col=footerfontcol))
  grid.text(author, x=unit(width-40,"points"),y=unit(20/height,"npc"),just="right",gp=gpar(fontsize=18,fontfamily=titlefont,fontface="bold", col=footerfontcol))
  img <- readPNG(logo)
